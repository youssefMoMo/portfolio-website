...
/**
 * Determine whether a given image src string is a usable, non-broken path.
 * A healthy path must be a non-empty string that begins with `/` (local asset)
 * or `http` (absolute URL). Inline data URIs (e.g. generated SVGs) are also
 * considered healthy since they are self-contained.
 */
function isImagePathHealthy(src: string | null | undefined): boolean {
...
/**
 * Migrate legacy Supabase storage URLs to local public assets and apply a
 * bulletproof SVG placeholder fallback for any item whose image path is
 * missing or broken after migration.
 *
 * Safety contract:
 *   1. If an item's image contains the OLD_SUPABASE_URL host, replace it
 *      with the corresponding local portfolio asset path.
 *   2. After the (potential) replacement, verify the resulting path with
 *      `isImagePathHealthy`. If it fails — empty string, null, malformed
 *      URL, unexpected mutation — intercept it and fall back to an inline
 *      SVG data URI generated by `svgPlaceholder()`. This guarantees the
 *      client layout never renders a broken <img> box.
 */
function fixPortfolioImages(content: PortfolioContent): PortfolioContent {
  return {
    ...content,
    items: content.items.map((item: PortfolioItem, i: number): PortfolioItem => {
      let image: string = item.image;

      // Step 1 — migrate legacy Supabase storage URLs to local assets.
      if (image && image.includes(OLD_SUPABASE_URL)) {
        image = `/images/portfolio/work${(i % 22) + 1}.png`;
      }

      // Step 2 — safety fallback: if the image path is still broken or
      // missing (empty string, null coerced to string, malformed path, etc.),
      // replace it with a deterministic inline SVG placeholder so the layout
      // never shows a torn-image icon.
      if (!isImagePathHealthy(image)) {
        image = svgPlaceholder(
          item.title ? String(item.title) : `Portfolio ${i + 1}`,
          600,
          400,
        );
      }
...
function normalizeReviewRow(r: Record<string, unknown>): Review {
  ...
  return {
    ...
    date:          String(r.date ?? (r.created_at as string | undefined)?.split("T")[0] ?? ""),
